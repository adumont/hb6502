...
parser.add_argument(
    "-b", "--breakpoint", help="set breakpoint (symbol)", default="do_BREAK"
)
...
def cpuThreadFunction(_ch, win, dbgwin, queue, queue_step, logfile):
    ...
    def disass_pane(mode, instr, _syms):
        ...
        log_registers = "A:{:02X}  X:{:02X}  Y:{:02X}  S:{:02X}  P:{}".format(
            mpu.a, mpu.x, mpu.y, mpu.sp, (itoa(mpu.p, 2).rjust(8, "0"))
        )
        ...
        curr_instr = render_instr(
            [
                f"{mpu.pc:04X}",
                f"{getByte(mpu.pc):02X}",
                f"{getByte(mpu.pc + 1):02X}",
                f"{getByte(mpu.pc + 2):02X}",
            ]
        )
        if symbols:
        ...
        if logfile:
            logfile.write(
                " | ".join([log_registers, log_forth_reg1, curr_instr]) + "\n"
            )
...
def main(stdscr):
    ...
    t = threading.Thread(
        target=cpuThreadFunction, args=("", cpuwin, dbgwin, queue, queue_step, logfile)
    )
    t.start()
